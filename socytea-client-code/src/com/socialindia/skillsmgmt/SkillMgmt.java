...
public class SkillMgmt extends ActionSupport implements SessionAware, ServletRequestAware, ServletResponseAware, ServletContextAware{
	...
	private Common commonObj = new CommonDao();
	  ...
	  public String execute() {
	    try {
			Commonutility.toWriteConsole("Skills ListDatatable  loading.......");
			String manualsearch = "";
			...
			try {
				...
				finaljj.put("servicecode", "SI380000");
				finaljj.put("data", dataJson);
				String jsonTextFinal = finaljj.toString();
				jsonTextFinal = EncDecrypt.encrypt(jsonTextFinal);
				String temp = "ivrparams=" + URLEncoder.encode(jsonTextFinal);
				String finalUrl = getText("socialindia.skillviewAll.skillviewAllmgmt");
				String response = commonObj.jsonRequest(finalUrl, temp);
				JSONObject json = new JSONObject(response);
				JSONObject json_data = json.getJSONObject("data");
				total = json_data.getInt("InitCount");
				ar = json_data.getJSONArray("skilldetails");
				JSONObject jsonList = new JSONObject();
				String skillName = "",categoryName = "",categoryId="", action = "", status = "",skillId="";
				int sno = 1;
				for(int i=0;i<ar.length();i++){	
					jsonList = null;
					jsonList = ar.getJSONObject(i);
					skillId = jsonList.getString("skillId");
					skillName = jsonList.getString("skillName");
					categoryId = jsonList.getString("categoryId");
					categoryName = jsonList.getString("categoryName");
					status = jsonList.getString("status");
					lvrrtnjary = new JSONArray();
					if (status.equalsIgnoreCase("0")) {
		            	action += "<a id=\"ideditid_" + skillId + "\" class=\"left\" style=\"margin-right: 4%;cursor: pointer;\" onclick=\"editskill('" + skillId + "','"+status+"');\">"
			            		+ "<i class=\"tooltipped "+getText("button.color.edit")+"\" data-tooltip=\"Edit\" data-delay=\"10\" data-position=\"bottom\"></i></a>";

						action +=" <a id=\"idcardid_"+ skillId +"\" onclick=\"activeaction('" + skillId + "','"+status+"');\" "
		            		   + "class=\"pointer\" style=\"margin-right: 10px;\">"
		            		   + " <i id=\"idcardi_deacticon_"+ skillId +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"bottom\"></i>"
							   + " <i id=\"idcardi_activeicon_"+ skillId +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"bottom\"></i>" 
		            		   + "</a>";
					} else if (status.equalsIgnoreCase("1")) {
						action += "<a id=\"ideditid_" + skillId + "\" class=\"left\" style=\"margin-right: 4%;cursor: pointer;\" onclick=\"editskill('" + skillId + "','"+status+"');\">"
			            		+ "<i class=\"tooltipped "+getText("button.color.edit")+"\" data-tooltip=\"Edit\" data-delay=\"10\" data-position=\"bottom\"></i></a>";

						action +=" <a id=\"idcardid_" + skillId + "\" onclick=\"deleteaction('" + skillId + "','"+status+"');\" "
		            		   + " class=\"pointer\" style=\"margin-right: 10px;\">"
							   +"  <i id=\"idcardi_activeicon_"+ skillId +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"bottom\"></i>"
							   + " <i id=\"idcardi_deacticon_"+ skillId +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"bottom\"></i>"
							   +"</a>";
					}
					if (status.equalsIgnoreCase("0")) {
						status ="<div id=\"statusval_"+ skillId + "\" >Deactive</div>";
					} else if (status.equalsIgnoreCase("1")) {
						status ="<div id=\"statusval_"+ skillId + "\">Active</div>";
					}
					lvrrtnjary.put(sno);
					lvrrtnjary.put(skillName);
