...
public class EducationServices extends ActionSupport{
	private String uniqueId;
	private String statusflg;
	private String iVOeducationname;
	...
	public String educationCreateFun() throws Exception {
			System.out.println("educationCreateFun create  loading.......");
			...
			try {	
				if(!iVOeducationname.equalsIgnoreCase("null")&& iVOeducationname!=null) {
				...
				finalUrl = getText("socialindia.educationcreation.action");
				response = commonObj.jsonRequest(finalUrl, temp);
				json = new JSONObject(response);
				msg= (String) json.get("message");
				statusCode = (String) json.get("statuscode");
				if (statusCode.equalsIgnoreCase("0")) {
					if (iVOeducationname == null) {
				        addFieldError("groupmang.groupName", getText("Error.usercreate.groupname"));
				    } else if (msg.equalsIgnoreCase("Education Type already exist.")) {
						iVOeducationname = "";
						alert.setCls("danger");
						alert.setMsg(getText("education.exist"));
						alertList.add(alert);
						sessionMap.put("alertList", alertList);
						return "input";
					} else {
						if (msg.equalsIgnoreCase("Education Type Created.")) {
							iVOeducationname = "";
							alert.setCls("success");
							alert.setMsg(getText("education.Create.Success"));
							alertList.add(alert);
							sessionMap.put("alertList", alertList);
							return "success";
						} else {
							alert.setCls("danger");
							alert.setMsg(getText("education.Create.Error"));
							alertList.add(alert);
							sessionMap.put("alertList", alertList);
							return "input";
						}
				...
				Commonutility.toWriteConsole("Step -1 : Exception Found "+getClass().getSimpleName()+".class : "+e);
			}finally{
	...
	public String deleteActionfun(){
		...
		try {
			...
			String finalUrl = getText("socialindia.education.deleteaction");
			String response = commonObj.jsonRequest(finalUrl, temp);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
			if (statusCode.equalsIgnoreCase("0")) {
				alert.setCls("success");
				alert.setMsg(getText("education.Delete.Success"));
				alertList.add(alert);
				return "success";
			} else {
				alert.setCls("danger");
				alert.setMsg(getText("education.Delete.Error"));
				alertList.add(alert);
				return "input";
			}
	...
	public String activeActionfun() {
		...
		try {
			...
			jsonTextFinal = EncDecrypt.encrypt(jsonTextFinal);
			String temp = "ivrparams=" + URLEncoder.encode(jsonTextFinal);
			String finalUrl = getText("socialindia.education.activeaction");
			String response = commonObj.jsonRequest(finalUrl, temp);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
			if (statusCode.equalsIgnoreCase("0")) {
				alert.setCls("success");
				alert.setMsg(getText("education.Active.Success"));
				alertList.add(alert);
				return "success";
			} else {
				alert.setCls("danger");
				alert.setMsg(getText("education.Active.Error"));
				alertList.add(alert);
				return "input";
			}
	...
	public String getiVOeducationname() {
		return iVOeducationname;
	}

	public void setiVOeducationname(String iVOeducationname) {
		this.iVOeducationname = iVOeducationname;
	}
