...
public class CyberPlatMgmt extends ActionSupport implements
...
ServletContextAware{
	private static final long serialVersionUID = 1L;
	  ...
	  public String execute() {
	    try {
	    	System.out.println("CyberPlatMgmt  loading.......");
	      ...
	      try {	       
			...
			String finalUrl =getText("socialindia.cyberPlatMgmt.action");
			String response=commonObj.jsonRequest(finalUrl, temp);
			System.out.println("======response======="+response);
			JSONObject json = new JSONObject(response);
			JSONObject json_data=json.getJSONObject("data");
			 total =  json_data.getInt("InitCount");
			 ar = json_data.getJSONArray("cyberplatedetails");
			 ...
			 for(int i=0;i<ar.length();i++){	
				 sno++;
					...
					String cyberId=jsonList.getString("cyberId");
					...
					String downloadfilePath=jsonList.getString("downloadfilePath");
					 action += "<a class=\"\" title=\"\"  role=\"button\" style=\"margin-left: 10px;cursor: pointer;\" onclick=\"deletepaygate('"
		                      + cyberId + "');\"><i class=\"tooltipped "+getText("button.color.delete")+"\" "
