...
public class titleMasterDTable extends ActionSupport implements
...
ServletContextAware{
	...
	public String execute() {
		Log logwrite = null;
		try {
			System.out.println("titleMasterDTable  loading.......");
			logwrite = new Log();
			...
			try {
				...
				for (int i = 0; i < ar.length(); i++) {				 
					...
					titleName = (String) Commonutility.toHasChkJsonRtnValObj(jsonList, "title_name");
					titleid=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "title_id");
					titlestatus=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "title_status");
					ja = null;
					ja = new JSONArray();
					if(titleName!=null){
						titleName = new CommonUtilsDao().toFirstCharUpper(titleName);
					}       
					if (titlestatus == 0) {
		            	   action +=" <a id=\"titleid_"+ titleid +"\" onclick=\"activeaction('" + titleid + "','"+titlestatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"titleid_deacticon_"+ titleid +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
			            		  + " <i id=\"titleid_activeicon_"+ titleid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            			  + " </a>";
					} else if (titlestatus == 1) {
			        	   action +=" <a id=\"titleid_" + titleid + "\" onclick=\"deleteaction('" + titleid + "','"+titlestatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"titleid_activeicon_"+ titleid +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
								  + " <i id=\"titleid_deacticon_"+ titleid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
				            	  + " </a>";
			        	  
					}
					if (titlestatus == 0) {
						statusflg = "<div id=\"statusval_" + titleid
								+ "\" >Deactive</div>";
					} else if (titlestatus == 1) {
						statusflg = "<div id=\"statusval_" + titleid
								+ "\">Active</div>";
			 ...
			 totalAfterFilter =  json_data.getInt("countAfterFilter");
	    	    ...
	    	    logwrite.logMessage("Step -1 : Exception Found : "+ex, "error", titleMasterDTable.class);
			} finally {
		    ...
		    System.out.println("Step 3 : Worktype MasterDTable Manage Table [End]"+new SimpleDateFormat("yyyy-MM-dd hh:mm:SSS").format(new Date()));
		 	logwrite.logMessage("Step 3 : Worktype MasterDTable Table [End] ", "info", titleMasterDTable.class);
		    getResponse();	   
	    } catch (Exception ex) {
	    	 System.out.println("Step -1 : Exception Found "+getClass().getSimpleName()+" 2 : "+ex);
	  	  	 logwrite.logMessage("Step -1 4: Exception Found : "+ex, "error", titleMasterDTable.class);
	    }
	    finally{
	    	logwrite = null;
	    }
