...
public class TomerchantServices extends ActionSupport{
	  private String uniqueId;
	  private String statusflg;
	  private String closereasonfield;
	  private String emailcommentfield;
	  private String tomerchantidfield;
	...
	public String ComplaintCreateFun() throws Exception{
		...
		try {	
			if(!disputemerchantobj.getDisputeTitle().equalsIgnoreCase("null")&& disputemerchantobj.getDisputeTitle()!=null) {
				...
				finalUrl = getText("socialindia.disputemgmt.tomerchantcreatecomplaint");
	...
	public String modifyServices() {
		System.out.println("------- disputemerchant Edit ------------------------");
		...
		try{
			...
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			finalUrl = getText("socialindia.disputemgmt.tomerchantview");
			...
			if (response != null && !response.equalsIgnoreCase("null")
				...
				if (ivIsJson) {
					...
					disputemerchantobj.setMerchantname((String)Commonutility.toHasChkJsonRtnValObj(json_data,"disp_tomerchantname"));
	...
	public String deleteCmpltAction(){
		...
		try{
			...
			finaljj.put("servicecode", "SI30005");		
			finaljj.put("data", obj);
			String jsonTextFinal = finaljj.toString();
			System.out.println(jsonTextFinal);
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			String finalUrl =getText("socialindia.disputemgmt.tomerchantcmpltdelete");
...
return SUCCESS;	
	...
	public String closeCmpltAction(){
		...
		try{
			...
			JSONObject finaljj=new JSONObject();
			finaljj.put("servicecode", "SI30002");		
			finaljj.put("data", obj);
			String jsonTextFinal = finaljj.toString();
			System.out.println(jsonTextFinal);
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			String finalUrl =getText("socialindia.disputemgmt.tomerchantcmpltclose");
			String response=commonObj.jsonRequest(finalUrl, temp);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
			if (statusCode.equalsIgnoreCase("0")) {
...
return SUCCESS;	
	...
	public String sendemailCmpltAction()
	{
		...
		try{
			String useridval=String.valueOf(sessionMap.get("USERID"));
			obj.put("crntusrloginid",useridval );
			obj.put("tomerchantid",Integer.parseInt(tomerchantidfield));
			...
			JSONObject finaljj=new JSONObject();
			finaljj.put("servicecode", "SI30006");		
			finaljj.put("data", obj);
			String jsonTextFinal = finaljj.toString();
			System.out.println(jsonTextFinal);
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			String finalUrl =getText("socialindia.disputemgmt.tomerchantcmpltemailsend");
			String response=commonObj.jsonRequest(finalUrl, temp);
			System.out.println("response-------------"+response);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
...
return SUCCESS;	
	...
	public String sendSMSCmpltAction()
	{
		...
		try{
			...
			JSONObject finaljj=new JSONObject();
			finaljj.put("servicecode", "SI30007");		
			finaljj.put("data", obj);
			String jsonTextFinal = finaljj.toString();
			System.out.println(jsonTextFinal);
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			String finalUrl =getText("socialindia.disputemgmt.tomerchantcmpltsmssend");
			String response=commonObj.jsonRequest(finalUrl, temp);
			System.out.println("response-------------"+response);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
...
public String getTomerchantidfield() {
	return tomerchantidfield;
}
public void setTomerchantidfield(String tomerchantidfield) {
	this.tomerchantidfield = tomerchantidfield;
}
