		...
		try {
			...
			if (response != null && !response.equalsIgnoreCase("null") && response.length() > 0) {
				...
				if (blacksnamearr != null && blacksnamearr.length() > 0) {
					...
					restRegObj.setStatusFlag(Integer.parseInt((String)Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson,"status")));
					System.out.println("Raj is here ...... ["+restRegObj.getStatusFlag()+"]");
...
//					restRegObj.setPstlcodename((String)Commonutility.toHasChkJsonRtnValObj(locObjRecvdataJson,"rest_pincodeName"));
	...
	public String updatedresidentActionFunction() {
		...
		try {
			...
			dataJson.put("status", restRegObj.getStatusFlag());
			System.out.println("Bhejne k pehle .... : "+restRegObj.getStatusFlag());
