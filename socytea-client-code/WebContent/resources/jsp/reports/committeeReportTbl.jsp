...
<body>
	...
	<div id="main">
		...
		<div class="wrapper">
			...
			<div id="breadcrumbs-wrapper">
				...
				<div class="header-search-wrapper grey hide-on-large-only">
					<i class="mdi-action-search active"></i> <input type="text" name="Search" class="header-search-input z-depth-2" placeholder="Explore Materialize">
				</div>
				<div class="container">
					<div class="row">
						<div class="col s12 m12 l12">
							<h5 class="breadcrumbs-title"><s:text name="Text.report.committee" /></h5>
							<ol class="breadcrumbs left">
								<li><a href="loginform"><s:text name="Dashboard" /></a></li>
								<li><a href="#"><s:text name="Text.Report.Management" /></a></li>
								<li class="active"><s:text name="Text.report.committee" /></li>

							</ol>
							<div class="right" id="xlspdfdownload">
							<a
							onclick="getencriptedData('<s:text name="socialindia.committeeReportXLS"/>');">
							<button id="addaccountbuttonid" class="<s:text name="button.color.new.create"/>"
								...
								<a
									onclick="getencriptedData('<s:text name="socialindia.committeeReportPDF"/>');">
		...
		<s:hidden id="societyId_session" value="%{#session.sSoctyId}"></s:hidden>
						...
						<table id="example" class="responsive-table display hoverable" cellspacing="0" width="100%">
							<thead>
									<tr class="<s:text name="datatable.heading.bgcolor"/>">
													...
													<th><s:text name="Text.adduser.town.ship.id" /></th>
													<th><s:text name="Text.adduser.society.id" /></th>
													<th><s:text name="Text.Form.EmailId" /></th>
													<th><s:text name="Text.adduser.mobnumber" /></th>												
													<th><s:text name="Text.country" /></th>
													<th><s:text name="Text.state" /></th>
													<th><s:text name="Text.city" /></th>
													<th><s:text name="Text.entryDate"/></th>
													
												    <th><s:text name="Text.accesschannel"/></th>
													<th><s:text name="Text.committerole"/></th>
	...
	<s:if test="(#session.sSoctyId!=null && #session.sSoctyId !='' && #session.sSoctyId!='-1')">
		<s:hidden id="sSoctyId" value="%{#session.sSoctyId}"></s:hidden>
	</s:if>	
	<s:else>
		<s:hidden id="sSoctyId" value="-1"></s:hidden>
	</s:else>
	...
	var oTable;
		...
		function createoTable() {
			...
			oTable = $('#example').DataTable({
								...
								"serverSide" : true,
								"displayLength" : 12,
								"ajaxSource" : "committeemgmtReportTbldata?fromdate="+ fromdate + "&todate=" + todate+"&sSoctyId="+sSoctyId+"&sTowshipId="+sTowshipId,
								"sorting" : [],
							...
							.DataTable(
									{
										...
										"serverSide" : true,
										"displayLength" : 12,
										"ajaxSource" : "committeemgmtReportTbldata?fromdate="
