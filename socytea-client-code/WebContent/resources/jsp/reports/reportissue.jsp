...
<body>
				...
				<div class="container">
					<div class="row">
						<div class="col s12 m12 l12">
							<h5 class="breadcrumbs-title"><s:text name="Text.report.reportissue" /></h5>
							<ol class="breadcrumbs left">
								<li><a href="loginform"><s:text name="Dashboard" /></a></li>
								<li><a href="#"><s:text name="Text.Report.Management" /></a></li>
								<li class="active"><s:text name="Text.report.reportissue" /></li>
			...
			<div class="input-field col s2">
					...
					<div class="input-field col s2">
						<button id="addaccountbuttonid"
							class="<s:text name="button.color.filter"/> tooltipped"
							...
							<thead>
								<tr class="<s:text name="datatable.heading.bgcolor"/>">
									<th><s:text name="S.No."></s:text></th>
													...
													<th><s:text name="text.entryDateTime"></s:text></th>
	...
	<s:form method="post" id="createreportissueid" name="createreportissueid">
		<s:hidden name="iVOreportissueid" id="iVOreportissueid"></s:hidden>
	...
	if (typeof(objects_scty) != "undefined" && objects_scty.indexOf(this.value)!=-1) {	
					...
					.DataTable(
							{
								...
								"serverSide" : true,
								"displayLength" : 10,
								"ajaxSource" : "reportissueTbldata?sSoctyId="+sSoctyId+"&sTowshipId="+sTowshipId,
								"sorting" : [],
		...
		function loadDataTableByFilter() {
			...
			oTable = $('#example').dataTable(
				...
				if (sTowshipId=='' && sSoctyId==''){
					swal("Select Township And  Society");	
				} else if (sTowshipId==''){
					 swal("Please Select Township");	
				} else if (sSoctyId=='') {
					 swal("Please Select Society");	
				}
			}
		...
		function viewreportissueaction(id)
		{
			$("#iVOreportissueid").val(id);
			$("#createreportissueid").attr("action","reportissuemstview");
			$("#createreportissueid").submit();
		}
		 ...
		 function solveactioncontent(id,mobno) {
				...
				swal({
					...
					$.ajax({
						type : 'post',
						datatype : 'html',
						url : 'solvereportissueaction',
						data : 'uniqueId=' + id +'&mobno='+ mobno,
						success : function(data) {
							$("#bookingappr_" + id).hide();
							$("#statusid_" + id).show();
							/* $("#imgmontid_" + id).attr("onclick", "activeactioncontent('" + id + "','"+tblflag+"')");*/
							$("#statusid_" + id).html("Solved"); 
							swal("solved !", "Record has been solved", "success");
						},
						error : function(d) {
							
							swal(statusval + "!", "Record has not been solved", "error");
						}
					});
					}else{
						
						$("#imgmontid_" + id).prop('checked',false).change();
						$("#imgmontcnt_lbl_" + id).html("&nbsp;&nbsp;Block Image");
					}
				});
			}	
	</script>
