...
public class CategoryMasterDTable extends ActionSupport implements
...
ServletContextAware{
	private static final long serialVersionUID = 1L;
	  ...
	  public String execute() {
	    try {
			System.out.println("CategoryMasterDTable  loading.......");
	      ...
	      try {	        
			...
			String finalUrl =getText("socialindia.configmgmt.categorydatable");			
			String response=commonObj.jsonRequest(finalUrl, temp);
			JSONObject json = new JSONObject(response);
			JSONObject json_data=json.getJSONObject("data");
			 total =  json_data.getInt("InitCount");
			 ar = json_data.getJSONArray("categorydetails");
			 ...
			 Integer categoryid,categorystatus;
				for (int i = 0; i < ar.length(); i++) {
					...
					categoryName = (String) Commonutility.toHasChkJsonRtnValObj(jsonList, "category_name");
					categoryid=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "category_id");
					categorystatus=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "category_status");
					//categoryList.add(new CategoryMasterTblVO(categoryid,categoryName,categorystatus));
					
					lvrrtnjary = null;
					lvrrtnjary = new JSONArray();
					statusflg =Integer.toString(categorystatus);
					
					action +="<a class=\"societystyle\"id=\"editcategoryid_"+ categoryid +"\" onclick=\"editcategory('"
	               			 + categoryid +"','"+categorystatus+"'  );\"><i class=\"tooltipped "+getText("button.color.edit")+"\" data-tooltip=\"Edit Category\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"left\"></i></a>";
					
					if (statusflg.equalsIgnoreCase("0")) {
						action +=" <a id=\"categoryid_"+ categoryid +"\" onclick=\"activeaction('" + categoryid + "','"+statusflg+"');\" "
		            		   + " class=\"pointer\" style=\"margin-right: 10px;\">"
		            		   + " <i id=\"categoryid_deacticon_"+ categoryid +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " <i id=\"categoryid_activeicon_"+ categoryid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " </a>";
					} else if (statusflg.equalsIgnoreCase("1")) {
						action +=" <a id=\"categoryid_" + categoryid + "\" onclick=\"deleteaction('" + categoryid + "','"+statusflg+"');\" "
		            		   + " class=\"pointer\" style=\"margin-right: 10px;\">"
		            		   +"  <i id=\"categoryid_activeicon_"+ categoryid +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
							   + " <i id=\"categoryid_deacticon_"+ categoryid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " </a>";
	        ...
	        System.out.println("Exception get totalcount---->> " + ex);
	    	  ...
	    	  dataJson = null; ar = null;  lvrrtnjary = null;
	      }	   
