...
public class knownusMasterDTable extends ActionSupport implements
...
ServletContextAware{
	...
	public String execute() {
		Log logwrite = null;
		try {
			System.out.println("knownusMasterDTable  loading.......");
			logwrite = new Log();
			...
			try {
				...
				finaljj.put("servicecode", "SI38000");
				finaljj.put("data", dataJson);
				String jsonTextFinal = finaljj.toString();
				jsonTextFinal = EncDecrypt.encrypt(jsonTextFinal);
				String temp = "ivrparams=" + URLEncoder.encode(jsonTextFinal);
				String finalUrl = getText("socialindia.configmgmt.knownusdatable");
				String response = commonObj.jsonRequest(finalUrl, temp);
				json = new JSONObject(response);
				json_data = json.getJSONObject("data");
				total = json_data.getInt("InitCount");
				ar = json_data.getJSONArray("knownusdetails");
				jsonList = new JSONObject();
				String knownusName = null;
				Integer knownusid, knownusstatus;							
				String statusflg = "";
				String action = "";
				int sno = start + 1;
				for (int i = 0; i < ar.length(); i++) {				 
					...
					knownusName = (String) Commonutility.toHasChkJsonRtnValObj(jsonList, "knownus_name");
					knownusid=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "knownus_id");
					knownusstatus=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "knownus_status");
					ja = null;
					ja = new JSONArray();
					if(knownusName!=null){
						knownusName = new CommonUtilsDao().toFirstCharUpper(knownusName);
					}       
					if (knownusstatus == 0) {
		            	   action +=" <a id=\"knownusid_"+ knownusid +"\" onclick=\"activeaction('" + knownusid + "','"+knownusstatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"knownusid_deacticon_"+ knownusid +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
			            		  + " <i id=\"knownusid_activeicon_"+ knownusid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            			  + " </a>";
					} else if (knownusstatus == 1) {
			        	   action +=" <a id=\"knownusid_" + knownusid + "\" onclick=\"deleteaction('" + knownusid + "','"+knownusstatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"knownusid_activeicon_"+ knownusid +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
								  + " <i id=\"knownusid_deacticon_"+ knownusid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
				            	  + " </a>";
			        	  
					}
					if (knownusstatus == 0) {
						statusflg = "<div id=\"statusval_" + knownusid
								+ "\" >Deactive</div>";
					} else if (knownusstatus == 1) {
						statusflg = "<div id=\"statusval_" + knownusid
								+ "\">Active</div>";
			 ...
			 totalAfterFilter =  json_data.getInt("countAfterFilter");
	    	    ...
	    	    logwrite.logMessage("Step -1 : Exception Found : "+ex, "error", knownusMasterDTable.class);
			} finally {
		    ...
		    System.out.println("Step 3 : Worktype MasterDTable Manage Table [End]"+new SimpleDateFormat("yyyy-MM-dd hh:mm:SSS").format(new Date()));
		 	logwrite.logMessage("Step 3 : Worktype MasterDTable Table [End] ", "info", knownusMasterDTable.class);
		    getResponse();	   
	    } catch (Exception ex) {
	    	 System.out.println("Step -1 : Exception Found "+getClass().getSimpleName()+" 2 : "+ex);
	  	  	 logwrite.logMessage("Step -1 4: Exception Found : "+ex, "error", knownusMasterDTable.class);
	    }
	    finally{
	    	logwrite = null;
	    }
