...
public class bloodtypeMasterDTable extends ActionSupport implements SessionAware, ServletRequestAware, ServletResponseAware,
ServletContextAware{
	...
	public String execute() {
		Log logwrite = null;
		try {
			System.out.println("bloodtypeMasterDTable  loading.......");
			logwrite = new Log();
			...
			try {
				...
				for (int i = 0; i < ar.length(); i++) {				 
					...
					bloodtypeName = (String) Commonutility.toHasChkJsonRtnValObj(jsonList, "bloodtype_name");
					bloodtypeid=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "bloodtype_id");
					bloodtypestatus=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "bloodtype_status");
					ja = null;
					ja = new JSONArray();
					if(bloodtypeName!=null){
						bloodtypeName = new CommonUtilsDao().toFirstCharUpper(bloodtypeName);
					}       
					if (bloodtypestatus == 0) {
		            	   action +=" <a id=\"bloodtypeid_"+ bloodtypeid +"\" onclick=\"activeaction('" + bloodtypeid + "','"+bloodtypestatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"bloodtypeid_deacticon_"+ bloodtypeid +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
			            		  + " <i id=\"bloodtypeid_activeicon_"+ bloodtypeid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            			  + " </a>";
					} else if (bloodtypestatus == 1) {
			        	   action +=" <a id=\"bloodtypeid_" + bloodtypeid + "\" onclick=\"deleteaction('" + bloodtypeid + "','"+bloodtypestatus+"');\" "
				            	  + " class=\"pointer\" style=\"margin-right: 10px;\">"
				            	  + " <i id=\"bloodtypeid_activeicon_"+ bloodtypeid +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
								  + " <i id=\"bloodtypeid_deacticon_"+ bloodtypeid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
				            	  + " </a>";
			        	  
					}
					if (bloodtypestatus == 0) {
						statusflg = "<div id=\"statusval_" + bloodtypeid
								+ "\" >Deactive</div>";
					} else if (bloodtypestatus == 1) {
						statusflg = "<div id=\"statusval_" + bloodtypeid
								+ "\">Active</div>";
			 ...
			 totalAfterFilter =  json_data.getInt("countAfterFilter");
	    	    ...
	    	    logwrite.logMessage("Step -1 : Exception Found : "+ex, "error", bloodtypeMasterDTable.class);
			} finally {
		    ...
		    System.out.println("Step 3 : Worktype MasterDTable Manage Table [End]"+new SimpleDateFormat("yyyy-MM-dd hh:mm:SSS").format(new Date()));
		 	logwrite.logMessage("Step 3 : Worktype MasterDTable Table [End] ", "info", bloodtypeMasterDTable.class);
		    getResponse();	   
	    } catch (Exception ex) {
	    	 System.out.println("Step -1 : Exception Found "+getClass().getSimpleName()+" 2 : "+ex);
	  	  	 logwrite.logMessage("Step -1 4: Exception Found : "+ex, "error", bloodtypeMasterDTable.class);
	    }
	    finally{
	    	logwrite = null;
	    }
