...
public class DocumentconfigServices extends ActionSupport{
	private String uniqueId;
	private String statusflg;
	private String iVOdocumentname;
	...
	public String documentconfigCreateFun() throws Exception{
			System.out.println("documentconfigCreateFun create  loading.......");
			...
			try {	
				if(iVOdocumentname!=null && !iVOdocumentname.equalsIgnoreCase("null") && !iVOdocumentname.equalsIgnoreCase("")) {
				...
				finalUrl = getText("socialindia.documentconfigcreation.action");
				...
				if (statusCode.equalsIgnoreCase("0")) {
					...
					if (iVOdocumentname == null) {
				        addFieldError("groupmang.groupName", getText("Error.usercreate.groupname"));
				      } else if (msg.equalsIgnoreCase("Document Type already exist.")) {
						iVOdocumentname = "";
						alert.setCls("danger");
						alert.setMsg(getText("documentconfig.exist"));
						alertList.add(alert);
						sessionMap.put("alertList", alertList);
						return "input";
				      } else {
						if (msg.equalsIgnoreCase("Document Type Created.")) {
							iVOdocumentname = "";
							alert.setCls("success");
							alert.setMsg(getText("documentconfig.Create.Success"));
							alertList.add(alert);
							sessionMap.put("alertList", alertList);
							return "success";
						} else {
							alert.setCls("danger");
							alert.setMsg(getText("documentconfig.Create.Error"));
							alertList.add(alert);
							sessionMap.put("alertList", alertList);
							return "input";
						}
				...
				Commonutility.toWriteConsole("Step -1 : Exception Found "+getClass().getSimpleName() +".class documentconfigCreateFun() : "+e);
			}finally{
	...
	public String deleteActionfun(){
			...
			try{
				...
				obj.put("documenttypeid", String.valueOf(uniqueId));
				...
				String finalUrl =getText("socialindia.documentconfig.deleteaction");
				String response=commonObj.jsonRequest(finalUrl, temp);
				JSONObject json = new JSONObject(response);
				String statusCode = (String) json.get("statuscode");
				if (statusCode.equalsIgnoreCase("0")) {
					alert.setCls("success");
					alert.setMsg(getText("documentconfig.Delete.Success"));
					alertList.add(alert);
				return "success";
				} else {
					alert.setCls("danger");
					alert.setMsg(getText("documentconfig.Delete.Error"));
					alertList.add(alert);
					return "input";
				}
  		
		} catch (Exception ex) {
			System.out.println("Step -1 : Exception Found "+getClass().getSimpleName() +".class deleteActionfun() : "+ex);
		} finally {
	...
	public String activeActionfun()
	{
		...
		try{
			...
			jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
			String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
			String finalUrl =getText("socialindia.documentconfig.activeaction");
			String response=commonObj.jsonRequest(finalUrl, temp);
			JSONObject json = new JSONObject(response);
			String statusCode = (String) json.get("statuscode");
			if (statusCode.equalsIgnoreCase("0")) {
				alert.setCls("success");
				alert.setMsg(getText("documentconfig.Active.Success"));
				alertList.add(alert);
			return "success";
			} else {
				alert.setCls("danger");
				alert.setMsg(getText("documentconfig.Active.Error"));
				alertList.add(alert);
				return "input";
			}
		
		}catch(Exception ex){
			System.out.println("Step -1 : Exception Found "+getClass().getSimpleName() +".class activeActionfun() : "+ex);
		} finally {
	...
	public String getiVOdocumentname() {
		return iVOdocumentname;
	}

	public void setiVOdocumentname(String iVOdocumentname) {
		this.iVOdocumentname = iVOdocumentname;
	}
