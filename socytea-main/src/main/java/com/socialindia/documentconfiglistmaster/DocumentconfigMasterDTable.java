package com.socialindia.documentconfiglistmaster;
...
public class DocumentconfigMasterDTable extends ActionSupport implements
...
ServletContextAware{
	private static final long serialVersionUID = 1L;
	 ...
	 // List<DoctypMasterTblVO> doccnfigList = new ArrayList<DoctypMasterTblVO>();
	...
	public String execute() {
		try {
			System.out.println("DoctypMasterTblVODTable  loading.......");
			String icol = "ivrBnDOC_TYP_ID";
			String table = "DoctypMasterTblVO";
			String manualsearch = "";
	      ...
	      try {	    
				...
				dataJson.put("currentloginid", String.valueOf(sessionMap.get("USERID")));
		  		...
		  		finaljj.put("servicecode", "SI18000");		
		  		finaljj.put("data", dataJson);	     
		        String jsonTextFinal = finaljj.toString();
		        jsonTextFinal=EncDecrypt.encrypt(jsonTextFinal);
				String temp="ivrparams="+ URLEncoder.encode(jsonTextFinal);			
				String finalUrl =getText("socialindia.configmgmt.documentconfigdatable");			
				String response=commonObj.jsonRequest(finalUrl, temp);
				json = new JSONObject(response);
				JSONObject json_data=json.getJSONObject("data");
				total = json_data.getInt("InitCount");
				ar = json_data.getJSONArray("documentconfigdetails");				
				jsonList = new JSONObject();
				String doccnfgName = null;
				Integer doccnfgid, doccnfgstatus;
				for (int i = 0; i < ar.length(); i++) {
					jsonList=null;
					jsonList=ar.getJSONObject(i);
					doccnfgName = (String) Commonutility.toHasChkJsonRtnValObj(jsonList, "documentcnfg_name");
					doccnfgid=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "documentcnfg_id");
					doccnfgstatus=(Integer) Commonutility.toHasChkJsonRtnValObj(jsonList, "documentcnfg_status");
					//doccnfigList.add(new DoctypMasterTblVO(doccnfgid,doccnfgName,doccnfgstatus));
					if(doccnfgName!=null){
						doccnfgName = new CommonUtilsDao().toFirstCharUpper(doccnfgName);
					}
					lvrrtnjary = null;
					lvrrtnjary = new JSONArray();
					statusflg =Integer.toString(doccnfgstatus);
					if (statusflg.equalsIgnoreCase("0")) {
						action +=" <a id=\"docconfid_"+ doccnfgid +"\" onclick=\"activeaction('" + doccnfgid + "','"+statusflg+"');\" "
		            		   + " class=\"pointer\" style=\"margin-right: 10px;\">"
		            		   + " <i id=\"docconfid_deacticon_"+ doccnfgid +"\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " <i id=\"docconfid_activeicon_"+ doccnfgid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " </a>";
					} else if (statusflg.equalsIgnoreCase("1")) {
						action +=" <a id=\"docconfid_" + doccnfgid + "\" onclick=\"deleteaction('" + doccnfgid + "','"+statusflg+"');\" "
		            		   + " class=\"pointer\" style=\"margin-right: 10px;\">"
		            		   + "  <i id=\"docconfid_activeicon_"+ doccnfgid +"\" class=\"tooltipped "+getText("button.color.active")+"\" data-tooltip=\"Click to Deactivate\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
							   + "  <i id=\"docconfid_deacticon_"+ doccnfgid +"\" style=\"display:none;\" class=\"tooltipped "+getText("button.color.deactive")+"\" data-tooltip=\"Click to Active\" data-delay=\""+getText("material.tooltip.delay")+"\" data-position=\"right\"></i>"
		            		   + " </a>";	        	 
					} else{
						
					}
					if (statusflg.equalsIgnoreCase("0")) {
						statusflg = "<div id=\"statusval_" + doccnfgid + "\" >Deactive</div>";
					} else if (statusflg.equalsIgnoreCase("1")) {
						statusflg = "<div id=\"statusval_" + doccnfgid + "\">Active</div>";
					}
				...
				System.out.println("Exception Found in "+getClass().getSimpleName()+".class 1 : " + ex);
			} finally {
				dataJson = null;
				ar = null;lvrrtnjary = null; jsonList = null; json = null;
			}
			...
			System.out.println("Exception Found in "+getClass().getSimpleName()+".class 2 : " + ex);
		} finally{
