...
public class TownshipMgmtReportDataTbl extends ActionSupport implements SessionAware, ServletRequestAware, ServletResponseAware,
ServletContextAware {
...
try {
		...
		if (sdtSortDir0 != null) {
			...
			try {
				...
				String finalUrl = getText("socialindia.townshipreportMng.loadDataTable");
				...
				for (int i = 0; i < ar.length(); i++) {			
					...
					Integer noOfFlats=jsonList.getInt("noOfFlats");	
					Integer noOfSociety=jsonList.getInt("noOfSociety");	
					String address=jsonList.getString("address");	
			//townShipMgmtList.add(new TownshipMstTbl(townshipid,townshipname, activationkey,countryname,statename,cityname));		
		        	...
		        	activekey = "<span id=\"newkey_"+townshipid+"\"> " + activekeymask+ "</span>";           
		            ja.put(lvrSno);
		            ja.put(townshipname);
		            ja.put(activationkey);
		            ja.put(countryname);
		            ja.put(statename);
		            ja.put(cityname);
		            ja.put(noOfFlats);
