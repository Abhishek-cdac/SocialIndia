            ...
            <div class="row">
              <div class="col s12 m12 l12">
                ...
                <ol class="breadcrumbs left">
                  ...
                  <div class="right">
                      <a href="uamusercreateform"><button type="button" style="width: 200px;" class=" <s:text name="button.color.new.create"/>" data-toggle="modal" data-target="#addnewaccount" data-position="bottom" data-delay="<s:text name="material.tooltip.delay"/>" data-tooltip="Create New User"><i class="mdi-social-person-add left"> </i>New User</button></a>
                      <a href="uamsiusercreateform"><button type="button" style="width: 200px" class=" <s:text name="button.color.new.create1"/>" data-toggle="modal" data-target="#addnewaccount" data-position="bottom" data-delay="<s:text name="material.tooltip.delay"/>" data-tooltip="Create SI User"><i class="mdi-social-person-add left"> </i> SI User</button></a>
                </div>                               						
              </div>
 ...
 <script type="text/javascript">
    	...
    	$('#societyId_txt_id').blur(function(){				
    	 ...
    	 function selectfieldfun(){
    			...
    			var locval = [{"id":"1","label":"All"},{"id":"2","label":"Email"},{"id":"3","label":"Mobile No."},{"id":"4","label":"Name"},{"id":"5","label":"Group Type"}];
			 ...
			 var township=$('#township_hidd_id').val();
				...
				oTable.destroy();
	 			oTable = $('#example').DataTable({
						...
						"serverSide" : true,
						"displayLength" : 10,
						"ajaxSource" :"addmerchantUserdatatable?townshipid="+ township+"&society="+society+"&slectfield="+slectfield+"&searchname="+searchname+"&searchval=1",
						"sorting" : [],
						"paginationType" : "full_numbers",
						"columnDefs" : [ {
							"sortable" : false,
		...
		toShowLoadingImgoverlay();
		 $.ajax({
				...
				success : function(data) {
		 		...
		 		error:function(d){tohideLoadingImgoverlay();
			 	}
			}); 
		// tohideLoadingImgoverlay();
	}
...
if(selectvalue=='Resident'){
	...
	$.ajax({
		...
		datatype : 'json',
		url : 'groupTypeUpdate',
		data : 'selectval='+selectval+'&selectId='+id+'&flg=mobile',
		success : function(data) {
	...
	$.ajax({
		...
		datatype : 'json',
		url : 'groupTypeUpdate',
		data : 'selectval='+selectval+'&selectId='+id+'&flg=both',
		success : function(data) {
			$("#labelGrpTyid_"+id).html(selectvalue);
			$("#editblockdivid_"+id).hide();
			$("#lableblockdivid_"+id).show();
