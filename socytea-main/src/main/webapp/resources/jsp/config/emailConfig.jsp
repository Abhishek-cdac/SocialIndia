					...
					<div class="col-md-8">
						...
						<div class="row">
							<div class="col-md-6">
								...
								<form method="post" name="formSmtpconfig" id="formSmtpconfig" >
									<table class="tblform">
										<tr>
											<td>
												<div>
													User Name<span class="mandatory">*</span>
												</div> <s:textfield name="emailconfObj.userName" id="usrnamCfg"
													cssClass="inp-form" value="%{emailconfObj.userName}"
													cssStyle="width:200px;" />
											</td>
											<td>
												<div>Password</div> <s:textfield
													name="emailconfObj.passWord" id="pswrdCfg"
													cssClass="inp-form" value="%{emailconfObj.passWord}"
													cssStyle="width:200px;" />
											</td>
										</tr>
										<tr>
											<td>
												<div>
													Host Name<span class="mandatory">*</span>
												</div> <s:textfield name="emailconfObj.hostName" id="hstnamCfg"
													cssClass="inp-form" value="%{emailconfObj.hostName}"
													cssStyle="width:200px;" />
											</td>
											<td>
												<div>
													Port Number<span class="mandatory">*</span>
												</div> <s:textfield name="emailconfObj.portNo" id="portnumCfg"
													cssClass="inp-form" value="%{emailconfObj.portNo}"
													cssStyle="width:200px;" />
											</td>
										</tr>
										<tr>
											<td>
												<div>
													Sender Email<span class="mandatory">*</span>
												</div> <s:textfield name="emailconfObj.fromId" id="fromidCfg"
													cssClass="inp-form" value="%{emailconfObj.fromId}"
													cssStyle="width:200px;" />
											</td>
											<td></td>
										</tr>
									...
									<s:hidden name="actiontypeonoff" id="actiontypeonoff" value="" />
									<div style="height: 7px"></div>
									<div>
										<input type="button" id="onid" name="onid" class="btnsubmit"
											value="ON" onclick="threadStopfn('on');"
											style="margin: 0 0 0 15px" /> <input type="button"
											id="offid" name="offid" class="btnsubmit" value="OFF"
											onclick="threadStopfn('off');" style="margin: 0 0 0 15px" />
									</div>
								</form>
...
"statcounter.com/counter/counter.js'></"+"script>");
	...
	<script type="text/javascript">
		function validate() {
			...
			$('#usrnamCfg').require("Employee name is required");
			$('#pswrdCfg').require("Password is required");
			$('#hstnamCfg').require("Host name is required");
			$('#portnumCfg').require("Port number is required").match("number",
					"Please enter valid port number");
			$('#fromidCfg').require("Please enter valid sender email address")
					.match("email", "Please enter valid sender email address");
			$("#fromidCfg").maxLength(50,
					"Sender Email should not exceeed max. of 50 characters");

			if ($('.error_box').html() == "") {
				$('.errordiv').hide();
				$("#formSmtpconfig").attr("action", "emailConfigUpdate");
				$("#formSmtpconfig").submit();
			}
		}
		function threadStopfn(onofval){
			$('#actiontypeonoff').val(onofval);
			$("#formSmtpconfig").attr("action", "threadStartStopAction");
			$("#formSmtpconfig").submit();
		}
	</script>
</body>
